...
-   [Data Summary: Music Economy](#pillar1) presents the data management of T1.1 (objective 1), T1.2 (objective 2), T1.3 (objective 3).
...
The main aim of the `DMP` is to inform potential users of the commonly understood data in the forms of datasets, datacubes, and databases of Open Music Europe visible via metadata and, whenever possible, with data catalogues and disseminated data. As required, we also make provisions for "other data", i.e. visualisation files, texts, blog posts, and more complex documents in lesser detail, as their access and use require less management information because their dissemination and communication are the focus of the Dissemination, Communication and Exploitation Plan. See [1.5 Data Dissemination: Open Music Observatory](#wp5) for further details.

::: callout-tip
## Purpose of data generation in relation to the project
...
The `Open Music Europe Data Management Plan` follows the *Horizon Europe Data Management Plan Template, Version 1* [@openaire_horizon_2021].
...
-   `wd:P140` (dimension) ↔ `qb:DimensionProperty`
-   `wd:P141` (measure) ↔ `qb:MeasureProperty`
-   `wd:P142` (attribute) ↔ `qb:AttributeProperty`
-   `wd:P146` (has code list) ↔ `sdmx:codeList`

### Structural Business Statistics (OpenMusE)

The *Structural Business Statistics (OpenMuse)* ([Q600](https://reprexbase.eu/openmusic/Item:Q600)) datacube is derived from the Eurostat [Enterprise statistics by size class and NACE Rev. 2 activity (from 2021 onwards)](https://ec.europa.eu/eurostat/databrowser/view/SBS_SC_OVW/) datacube with filtering out relevant industries, and establishing the total number of enterprises and their total turnover.

It contains two indicators: [number of enterprises](https://reprexbase.eu/openmusic/Item:Q605) and [net turnover of enterprises in million euros](https://reprexbase.eu/openmusic/Item:Q604), aggregated according to two approximate definitions of the music industry. The *CICERONE* flag marks observations that fall within the minimum or expanded NACE ranges defined in the CICERONE project, while the other (the *OpenMusE SBS* flag) marks observations belonging to the core or extended music-sector definitions developed in OpenMusE. These flags enable consistent extraction, comparison, and modelling of music-related economic activity across otherwise incompatible datasets.

::: callout-note
This part is for illustration only, and does not need repetition in the DMP.

The dataset [Q600.ttl](https://reprexbase.eu/openmusic/Special:EntityData/Q600.ttl) (also available as [Q600.nt](https://reprexbase.eu/openmusic/Special:EntityData/Q600.nt), [Q600.jsonld](https://reprexbase.eu/openmusic/Special:EntityData/Q600.jsonld), [Q600.json](https://reprexbase.eu/openmusic/Special:EntityData/Q600.json), and [Q600.rdf](https://reprexbase.eu/openmusic/Special:EntityData/Q600.rdf) in XML) represents a machine-readable model of the Eurostat table *“* *Structural Business Statistics (OpenMuse)* *”*. It was reconstructed in the OpenMusic Wikibase `https://reprexbase.eu/openmusic/`) to demonstrate alignment between European statistical metadata (SDMX) and linked-data publication formats used in the cultural-heritage domain.
...
-   [NACE economic activity (dimension)](https://reprexbase.eu/openmusic/Item:Q599)

-   [time (dataset dimension)](https://reprexbase.eu/openmusic/Item:Q472 "Item:Q472") *currently not added, but it would be absolutely essential*

-   [geopolitical entity (dimension)](https://reprexbase.eu/openmusic/Item:Q467 "Item:Q467") *not present now but would be absolutely necessary*

The measured variables of the datacube:

-   [number of enterprises](https://reprexbase.eu/openmusic/Item:Q605)
-   [net turnover of enterprises in million euros](https://reprexbase.eu/openmusic/Item:Q604),
...
-   [unit of measure (attribute)](https://reprexbase.eu/openmusic/Item:Q469 "Item:Q469") (not present now)

-   [frequency (attribute)](https://reprexbase.eu/openmusic/Item:Q470 "Item:Q470") (not present now)

-   [OpenMusE SBS music-sector flag](https://reprexbase.eu/openmusic/Item:Q602)

-   [CICERONE music classification flag](https://reprexbase.eu/openmusic/Item:Q602)
...
Our Consortium pledged to follow the Open Policy Guidelines (Level 3). [@opa_guidelines; @OPA_guardia_et_al_2020]. Within the Horizon Europe research framework programme, some open science practices are mandatory for all beneficiaries per the grant agreement, including using the FAIR principles [@horizon_europe_guide_2023, p43]. Compliance with the FAIR principles is laid out in Parts 2-5.
...
In *Open Music Europe*, we chose to comply with such an additional standard, the *Open Policy Analysis Guidelines* [@OPA_guardia_et_al_2020], which are a practical standard intended to one of our core target groups: cultural policymakers.
:::
...
## 1.4.1 Data Summary: Music Economy {#pillar1}

::: callout-tip
## Data Generated & Reused: Music Economy
...
The dataset is part of the [Music Economy Pillar](https://reprexbase.eu/openmusic/Item:Q585) of the Open Music Observatory.
...
## 1.5 Data Dissemination: Observatory {#wp5}
...
+====================================+=============================================================================+========================================================================================================================================+===============================================================================================================+
| survey mircodata datasets          | Collected by SINUS and REPREX.                                              | Sensitive data is stored by collecting entity.                                                                                         | No access to sensitive microdata.                                                                             |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    | Reused under the Open Data Directive.                                       | Statistical microdata on GitHub repositories (temporary) and Zenodo (long-term)                                                        | Non-identifiable datasets will be free to reuse with open data license.                                       |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             |                                                                                                                                        | reused data will be made further reusable on similar terms we receive them.                                   |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| administrative records             | ALOADED, ARTISJUS, SOZA, MUSICAUTOR and third party administrative records. | Sensitive data is stored by collecting entity.                                                                                         | No access to sensitive microdata.                                                                             |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    | Processed by Consortium members.                                            | Non-sensitive and limited subsets from M17 in Digital Music Observatory as linked open data.                                           | Limited access to microdata with data linking. Linking is possible with individual licenses.                  |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| statistical and indicator datasets | Reused from various open government sources.                                | [Temporary access on GitHub](https://github.com/dataobservatory-eu/wp1-to-music-observatory) (pre-processed and not human-controlled.) | Released as open data.                                                                                        |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    | Processed from Consortium member microdata by Consoritum.                   | [Open Music Observatory on Zenodo](https://zenodo.org/communities/music_observatory/records?q=&l=list&p=1&s=10)                        |                                                                                                               |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             | From M17 in Digital Music Observatory API.                                                                                             |                                                                                                               |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| data visualisations                | Created by members of the Consortium or Consortium.                         | FigShare open science repository.                                                                                                      | CC-BY license.                                                                                                |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| software code                      | UTU, REPREX in open collaboration with qualifying volunteers.               | GitHub (development versions)                                                                                                          | Free access, with open source license statement for each software.                                            |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             | Comprehensive R Archive Network (CRAN) peer-reviewed releases.                                                                         |                                                                                                               |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             | Zenodo (development and peer-reviewed releases.)                                                                                       |                                                                                                               |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| statistical processing code        | UTU, REPREX in collaboration with microdata owners.                         | [GitHub](https://github.com/dataobservatory-eu/wp1-to-music-observatory)\                                                              | Whenever possible, released as software.                                                                      |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             |                                                                                                                                        | Redactions are possible for data security purposes.                                                           |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| documents                          | Open Music Europe Consortium and its members                                | GitHub (see references)                                                                                                                | Free access for non-commercial use.                                                                           |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             | Final versions on Zenodo (and in journals)                                                                                             |                                                                                                               |
|                                    |                                                                             |                                                                                                                                        |                                                                                                               |
|                                    |                                                                             | This is the example                                                                                                                    |                                                                                                               |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
...
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+
| metadata                           | Open Music Europe Consortium and its members                                | See details of `DMP`.                                                                                                                  | CC0                                                                                                           |
+------------------------------------+-----------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------+

# 2. FAIR data (NOT ON DATASET LEVEL) {#FAIR-data}
...
![PID for Visualizations: All our visualisations will have a DOI on FigShare](png/Music_value_chain.png)
...
All our data will be provided with the mandatory metadata of the *DataCite 4.4* standard [@datacite_4_4], and in almost all cases, all recommended metadata, too.
...
Our main, long-term repository for data is Zenodo. The project's 5.1 deliverable, the Open Music Observatory, had a minimum viable product prototype, the Digital Music Observatory, which has had a community on Zenodo since 2021. [@digital_music_observatory_zenodo]
...
#### Property alignment between OpenMusic Wikibase and SDMX / RDF Data Cube {.unnumbered}

+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| OpenMusic property                                                               | Function / role                                                 | SDMX equivalent                       | RDF Data Cube equivalent        | Example entity / comment                                                 |
+==================================================================================+=================================================================+=======================================+=================================+==========================================================================+
| [om:P140](https://reprexbase.eu/openmusic/Property:P140) (“dimension (dataset)”) | Identifies one axis of observation (e.g., time, sex, education) | `sdmx:DimensionProperty`              | `qb:DimensionProperty`          | `wd:Q456` “sex (dimension)”, `wd:Q472` “time (dimension)”                |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P141](https://reprexbase.eu/openmusic/Property:P141) (“measure (dataset)”)   | Quantitative variable observed                                  | `sdmx:MeasureProperty`                | `qb:MeasureProperty`            | `wd:Q471` “persons practising artistic activities (measure)”             |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P142](https://reprexbase.eu/openmusic/Property:P142) (“attribute (dataset)”) | Qualifying metadata (unit, frequency, status)                   | `sdmx:AttributeProperty`              | `qb:AttributeProperty`          | `wd:Q469` “unit of measure”, `wd:Q470` “frequency”                       |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P146](https://reprexbase.eu/openmusic/Property:P146) (“has code list”)       | Links a dimension or concept to its controlled vocabulary       | `sdmx:codeList`                       | —                               | `wd:Q463` “CL_SEX”                                                       |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P145](https://reprexbase.eu/openmusic/Property:P145) (“publisher”)           | Identifies data provider                                        | `sdmx:DataProvider`                   | `dct:publisher`                 | `wd:Q449` “Eurostat”                                                     |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P95](https://reprexbase.eu/openmusic/Property:P95) (“DOI”)                   | Persistent identifier                                           | `sdmx:Identifier`                     | `dct:identifier`                | `10.2908/ILC_SCP07`                                                      |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| [om:P130](https://reprexbase.eu/openmusic/Property:P130) (“has access point”)    | Access URL for dataset                                          | —                                     | `dcat:accessURL` / `schema:url` | [`https://doi.org/10.2908/ILC_SCP07`](https://doi.org/10.2908/ILC_SCP07) |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| `prov:wasDerivedFrom`                                                            | Provenance record                                               | `sdmx:source`                         | `prov:wasDerivedFrom`           | Original Eurostat dataset source                                         |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+
| `cc:license`                                                                     | Rights and licensing                                            | `sdmx:DataSetAnnotation` (conceptual) | `dct:license`                   | `CC0 1.0 Public Domain Dedication`                                       |
+----------------------------------------------------------------------------------+-----------------------------------------------------------------+---------------------------------------+---------------------------------+--------------------------------------------------------------------------+

#### Class alignment between OpenMusic Wikibase, SDMX, and W3C RDF Data Cube {.unnumbered}

+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| # OpenMusic / Wikibase class                                                                                                         | # Equivalent SDMX concept      | # Equivalent W3C Data Cube / W3C class | # Description                                                                            |                            |
|                                                                                                                                      |                                |                                        |                                                                                          |                            |
| [om:Q285](https://reprexbase.eu/openmusic/Item:Q285) (“data set”)                                                                    | `sdmx:DataStructureDefinition` | `qb:DataStructureDefinition`           | Defines the structure of dimensions, measures, and attributes within the dataset.        |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
|                                                                                                                                      |                                |                                        |                                                                                          |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q446](https://reprexbase.eu/openmusic/Item:Q446) (“Persons practising artistic activities by sex, age, education and frequency”) | `sdmx:DataSet`                 | `qb:DataSet`                           | Represents the Eurostat dataset reconstructed as RDF.                                    |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q449](https://reprexbase.eu/openmusic/Item:Q449) (“Eurostat”)                                                                    | `sdmx:DataProvider`            | `dct:publisher`                        | Organisation responsible for providing and maintaining the dataset.                      |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q456](https://reprexbase.eu/openmusic/Item:Q456) (“sex (dimension)”)                                                             | `sdmxdim:SEX`                  | `qb:DimensionProperty`                 | Dimension variable defining the sex of the observed population.                          |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q467](https://reprexbase.eu/openmusic/Item:Q467) (“geopolitical entity (dimension)”)                                             | `sdmxdim:REF_AREA`             | `qb:DimensionProperty`                 | Dimension variable defining the geographic area (country or region).                     |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q472](https://reprexbase.eu/openmusic/Item:Q472) (“time (dataset dimension)”)                                                    | `sdmxdim:TIME_PERIOD`          | `qb:DimensionProperty`                 | Dimension variable defining the reference time period.                                   |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q466](https://reprexbase.eu/openmusic/Item:Q466) (“educational attainment (dimension)”)                                          | `sdmxdim:EDUCLEVEL`            | `qb:DimensionProperty`                 | Dimension representing the highest level of education attained.                          |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q471](https://reprexbase.eu/openmusic/Item:Q471) (“persons practising artistic activities (measure)”)                            | `sdmx:Measure`                 | `qb:MeasureProperty`                   | Quantitative variable representing the observed value (number or percentage of persons). |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q469](https://reprexbase.eu/openmusic/Item:Q469) (“unit of measure (attribute)”)                                                 | `sdmx:UNIT_MEASURE`            | `qb:AttributeProperty`                 | Attribute specifying the unit of measurement (e.g., percentage of persons).              |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q470](https://reprexbase.eu/openmusic/Item:Q470) (“frequency (attribute)”)                                                       | `sdmxdim:FREQ`                 | `qb:AttributeProperty`                 | Attribute describing the observation’s reporting frequency (annual, quarterly, etc.).    |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q463](https://reprexbase.eu/openmusic/Item:Q463) (“CL_SEX”)                                                                      | `sdmx:CodeList`                | `skos:ConceptScheme`                   | Controlled vocabulary providing coded values for the “sex” dimension.                    |                            |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q604](https://reprexbase.eu/openmusic/Item:Q605) net turnover (million euro)                                                     | `ESTAT:INDIC_SBS(10.0)`        |                                        |                                                                                          | Eurostat SBS 4.1 indicator |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q605](https://reprexbase.eu/openmusic/Item:Q605) enterprise number                                                               | `ESTAT:INDIC_SBS(10.0)`        |                                        |                                                                                          | Eurostat SBS 4.1 indicator |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q602](https://reprexbase.eu/openmusic/Item:Q605) OpenMusE SBS music-sector flag \|                                               | `OpenMusE defined`             |                                        |                                                                                          | D1.2 reference             |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+
| [om:Q601](https://reprexbase.eu/openmusic/Item:Q605) CICERONE music classification flag                                              | `OpenMusE defined`             |                                        |                                                                                          | D1.2 reference             |
+--------------------------------------------------------------------------------------------------------------------------------------+--------------------------------+----------------------------------------+------------------------------------------------------------------------------------------+----------------------------+

In WP1 we will use, beyond the SDMX taxonomies and ontologies the new ESCO 2.0 ontology [@esco_ontology_2_0].
...
In T3.2, we will aim to apply more and more of the DDI Standards, starting with the `DDI-Codebook 2.5` standard [@DDI-Codebook_2_5]. Later versions of this `DMP` will guide on the introduction of further DDI standards.
...
-   **Statistical data** use SDMX and W3C RDF Data Cube provenance elements (`prov:wasDerivedFrom`, `dct:source`);

-   **Survey data** follow the DDI standard for variable- and respondent-level documentation;
...
Observations and entities are uniquely identified with **persistent identifiers (PIDs)**, typically HTTP URIs resolving to the OpenMusic Wikibase ([`https://reprexbase.eu/openmusic/`](https://reprexbase.eu/openmusic/)).\
...
-   **Metadata** — All metadata describing datasets, variables, and provenance are released under **CC0 1.0 Public Domain Dedication**.\
    This allows unrestricted reuse, harvesting, and aggregation in catalogues such as Zenodo, OpenAIRE, and the ECCCH data space.

-   **Open data** — Aggregated or derived datasets produced by the project (e.g. statistical indicators, anonymised survey summaries, or cultural statistics) are released under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.en), unless specific partner or source conditions require another licence.\
    Each dataset’s licence is declared explicitly in its metadata (`dct:license`) under the [om:P148](https://reprexbase.eu/openmusic/Property:P148) property.
...
::: callout-note
## Internal peer review short description
...
+===========================+=====================================================+
| `dct:title`               | "Open Music Europe Example Dataaset"\@en            |
+---------------------------+-----------------------------------------------------+
| `dcat:keyword`            | "music"\@en, "musique"\@fr, "payments"\@en          |
+---------------------------+-----------------------------------------------------+
| `dct:creator`             | Jane Doe                                            |
+---------------------------+-----------------------------------------------------+
| `dct:isseud`              | "2021-06-01"\^\^xsd:date                            |
+---------------------------+-----------------------------------------------------+
...
+---------------------------+-----------------------------------------------------+
| `dct:publisher`           | open-music-europe                                   |
+---------------------------+-----------------------------------------------------+
...
Q600: *Structural Business Statistics (OpenMuse)* [@sbs_openmuse_table3]
